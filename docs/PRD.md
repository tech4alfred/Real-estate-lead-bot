...
Percentage of conversations transferred to humans because AI could not
safely proceed.

24. MVP Scope

For the first working version, I recommend not building the entire
vision.

MVP should include: React Chat Interface ↓ FastAPI ↓ n8n ↓ AI Extraction
↓ Lead Qualification ↓ PostgreSQL/MySQL ↓ Sales Notification ↓ Basic
Sales Dashboard MVP Features Customer chat Message processing AI intent
extraction Lead information extraction Missing-field detection Lead
scoring Lead classification SQL persistence Sales notification Lead
dashboard Lead assignment Status updates Follow-up notes Conversation
history 25. Future Scope

After MVP validation:

Phase 2 WhatsApp integration Property inventory integration Automated
property recommendations Email integration SMS Calendar integration
Advanced sales dashboard Phase 3 Lead prediction AI-assisted sales
recommendations Automated follow-up sequences Agent performance
analytics Customer segmentation Property matching engine Phase 4

Potentially:

CRM + AI Sales Copilot + Property Matching + Lead Intelligence +
Analytics 26. Key Product Principles
...
Understanding Extraction Classification Conversation Summarization

Use traditional code/workflows for:

Validation Scoring Authorization Persistence State transitions
Calculations Principle 6 --- n8n for Orchestration

n8n coordinates workflows and integrations rather than becoming the
application's entire backend.

Principle 7 --- Fail Gracefully

AI, APIs, databases and integrations will occasionally fail. The system
...
The MVP can be considered functional when a user can perform the
following:
...
"I want to buy a 3-bedroom apartment in Lekki. My budget is ₦80 million
and I want to move within two months."

System should:

Receive message ↓ Extract information ↓ Create/update lead ↓ Calculate
score ↓ Classify lead ↓ Store lead ↓ Respond to customer ↓ Notify sales
team Scenario 2 --- Incomplete Lead

Customer:

"I want a 3-bedroom apartment in Lekki."

System should recognize missing information and ask an appropriate
follow-up question rather than prematurely qualifying the lead.
...
The system must combine the messages into the same conversation/lead
context.
...
If AI processing fails, the system should not silently lose the
customer's enquiry.
...
React application works FastAPI API is operational n8n workflows are
operational SQL database is configured Lead schema is implemented
Customer messages can be received AI can extract defined fields Missing
fields can be identified Lead scoring works Lead classification works
Leads are persisted Sales notifications work Sales users can view leads
Sales users can update lead status Follow-up activity can be recorded
Conversation history is available Error handling exists
Authentication/authorization exists for internal users Automated tests
cover critical workflows Documentation is updated Deployment process is
documented 29. Requirements Traceability

One of the things I'd introduce from the beginning is traceability.

Every major requirement should eventually map to implementation and
testing.

For example:

PRD Requirement ↓ Technical Specification ↓ API Contract ↓
Implementation ↓ Test

Example:

FR-009 Lead Scoring ↓ TECH-004 Qualification Engine ↓ POST
/api/v1/leads/{id}/qualify ↓ QualificationService ↓ test_lead_scoring.py

This becomes particularly powerful when using AI coding agents because
an agent can be given a precise task such as:

Implement FR-009 according to TECH-004 and ensure all associated tests
pass.

30. Initial Product Architecture Direction

At the PRD level, we're intentionally not locking every technical detail
yet.
...
The important distinction is that this is the product direction, not yet
the final System Architecture. We'll make those decisions properly in
Document #2.

31. Open Decisions Before Architecture

There are a few things deliberately left as decisions rather than
assumptions:

Decision Current Direction Frontend React Backend FastAPI Automation n8n
Primary DB SQL Operational data/reporting Google Sheets AI LLM-based
processing Architecture style Modular architecture; avoid premature
microservices Lead scoring Deterministic rules AI role Extraction +
understanding + response Human escalation Required Property inventory
Future/optional integration WhatsApp Future integration unless selected
for MVP Authentication Required for sales/admin interface

These should be resolved or refined in the System Architecture Document
rather than prematurely hard-coded into the PRD.

32. PRD → Next Engineering Documents

Now that the PRD establishes the product truth, the next documents
should derive from it in this order:
